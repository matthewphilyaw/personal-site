...
const _buildPath = path.join(_rootPath, 'dist/public');
...
const entries = {};
getDirectoryNames(path.join(_rootPath, 'src', 'entries'))
    .forEach((dir, index) => {
      const entryName = dir;
      const entryFile = `${entryName}.ts`;
      const entryPath = path.join(_rootPath, 'src', 'entries', dir, entryFile);
...
module.exports = {
  context: path.join(_rootPath, 'src'),
  entry: {
